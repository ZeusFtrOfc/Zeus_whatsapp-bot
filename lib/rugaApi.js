...
const jam = async (url) => new Promise((resolve, reject) => {
    axios.get(`https://arugaz.my.id/api/edu/enwiki?query=${url}`)
    .then((res) => {
        const jamd = `from: ${res.data.from}\n\nto: ${res.data.result.to}\n\npageid: ${res.data.result.pageid}\n\ntitle: ${res.data.result.title}\n\nteks: ${res.data.result.extract}`
        resolve(jamd)
    })
    .catch((err) =>{
        reject(err)
...
const spamcall = async (notlp) => new Promise((resolve, reject) => {
	axios.get(`https://mhankbarbar.herokuapp.com/api/spamcall?no=${notlp}`)
	.then((res) => {
