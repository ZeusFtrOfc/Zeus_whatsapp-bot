...
case 'liputan6':
					...
					rugaapi.liputan()
					.then(async ({ result }) => {
						let newslp = '-----[ *BERITA LIPUTAN-6* ]-----'
						for (let i = 0; i < result.length; i++) {
							newslp += `\n\n• *Judul :* ${result[i].title}\n• *Time :* ${result[i].time}\n• *Url :* ${result[i].url}\n• *Berita :* ${result[i].ket}\n• category :* ${result[i].category}\n\n=_=_=_=_=_=_=_=_=_=_=_=_=\n\nBy : Zeus`
						}
						await aruga.reply(from, newslp, id)
						console.log('Succes Sending liputan')
					})
					.catch(async (err) => {
						console.error(err)
						aruga.reply(from, 'Error....', id)
...
case 'report':
case 'lapor':
if (args.length === 0) return aruga.reply(from, 'Kirim perintah *#report [bug pada bot*', id)
		aruga.reply(from, mess.wait, id)
		const reporter = body.slice(8)
		await aruga.sendText(ownerNumber, `Laporan bug dari : *${pushname}*\n\nBug : *${reporter}*\n\nNomer pelapor : *wa.me/${serial.replace(/@c.us/g, '')}*`)
		aruga.reply(from, 'Laporan berhasil dikirim ke Owner Bot!\nTerimakasih sudah melapor..\nmohon tunggu balasan dari owner!\n\n\n~ZeusXz Bot', id)
		break
case 'filmapikdownload':
						...
						await aruga.reply(from, filpikd, id)
						console.log('Succes Sending List Nekopoi')
					})
					.catch(async (err) => {
						console.error(err)
					...
					break
					case 'nickml':
            if (args.length == 0) return aruga.reply(from, `Ketik #nickml id ml nya ` ,  id)
            await aruga.reply(from, mess.wait, id)
            const nick = await axios.get(`http://hujanapi.xyz/api/nickml?id=${q}&apikey=${apikeyz}`)
            await aruga.reply(from, nick.data.result , id)
            break
                  case 'nickff':
           if (args.length == 0) return aruga.reply(from, `Ketik #nickff id ff nya ` ,  id)
            await aruga.reply(from, mess.wait, id)
            const nic1k = await axios.get(`http://hujanapi.xyz/api/nickff?id=${q}&apikey=${apikeyz}`)
            await aruga.reply(from, nick1.data.result , id)
            break
...
case 'neko18':
 await aruga.reply(from, mess.wait, id)
                        ...
                        aruga.sendImage(from, nsfwn.result, `NsfwNeko${ext}`, 'NsfwNeko!', id)
                         break
...
case 'iri':
		case 'Iri':
			...
			break
case 'prank':
		    aruga.sendPtt(from, './media/desahan.mp3', id)
			break
case 'pale':
		    aruga.sendPtt(from, './media/pale.mp3', id)
			break
case 'kuntilanak':
		    aruga.sendPtt(from, './media/kunti.mp3', id)
			break
...
aruga.reply(from, mess.wait, id)
            ...
            await aruga.reply(from, hota, id)
            .catch(() => {
                aruga.reply(from, 'Ada yang Error!', id)
            })
break 
case 'cakep':
            if (!isGroupMsg) return aruga.reply(from, 'Maaf, perintah ini hanya dapat dipakai didalam grup!', id)
            const gmekk = await aruga.getGroupMembersId(groupId)
            ...
            aruga.sendTextWithMentions(from, mmkkkk, id)
            break
...
case 'xnxxapp':
                    aruga.reply(from, mess.wait, id)
          / await  aruga.sendFileFromUrl(from, `https://piyobot.000webhostapp.com/XXNX%20Mod%20agustusan%20(SFILE.MOBI).apk`, 'xxnx.apk', id)
           await  aruga.reply(from, 'Nih\'s Apknya coli sonoo..', id)
           .then(() => console.log('Success send apk'))
break
                case 'nekopoiapp':
          aruga.reply(from, mess.wait, id)
           await  aruga.sendFileFromUrl(from, `https://bit.ly/nekopoiapp`, 'nekopoi.apk', `Nih apk nya buat nonton hentai..`,  id)
           .then(() => console.log('Success send apk'))
break        
case 'puisirandom':
               aruga.reply(from, mess.wait, id)
                await  aruga.sendFileFromUrl(from, `https://api.vhtear.com/puisi_image&apikey=${vhtearkey}`)
                break
case 'lord'://Piyobot
    ...
    let nadhira = lord[Math.floor(Math.random() * lord.length)]
     aruga.sendFileFromUrl(from, nadhira, 'lord.mp4','Nih ngab', id)
    ...
    break
case 'trumptweet':
                if (args.length == 0) return  aruga.reply(from, `Kirim perintah #trumptweet [ Teks ], contoh #trumptweet ZeusXZ`, id)
                 aruga.reply(from, mess.wait, id)
                ...
                if (tumh.message.endsWith('.png')) {
                 ...
                 aruga.sendFileFromUrl(from, tumh.message, `Nekonime${ext}`, 'Noh mhank', id)
                                break
case 'peluk': //thanks to SASHA BOT
 if (args.length == 0) return  aruga.reply(from, `Kirim perintah #peluk @tag member`, id)
               
    const janjing = author.replace('@c.us', '')
    await aruga.sendGiphyAsSticker(from, 'https://media.giphy.com/media/od5H3PmEG5EVq/giphy.gif')
    aruga.sendTextWithMentions(from, `@` + janjing + ' *peluuuk tu anak orangg:"* ')
    break;
case 'chika'://piyobot
if (!isPrem) return aruga.reply(from, 'Command Premium!\ngagal! Maaf anda belum terdaftar sebagai user premium\nsilahkan chat owner bot untuk mendaftar.', id)
    ...
    aruga.sendFileFromUrl(from, `https://piyobot.000webhostapp.com/${chikax}.mp4`, 'chika.mp4', 'Nih tod', id)
...
case 'valentine':     
 ...
 if (isMedia && type === 'image' || isQuotedImage) {
                            ...
                            const foto = await aruga.getProfilePicFromServer(sender.id)
                            ...
                            rugaapi.valentine(nama, pasangan, fotoMu, fotoPasangan)
                                .then(async ({ result }) => {
                                    await aruga.sendFileFromUrl(from, result.imgUrl, `${nama}_${pasangan}.jpg`, '', id)
                                        .then(() => console.log('Success creating image!'))
                                })
                                .catch(async (err) => {
                                    console.error(err)
                                    await aruga.reply(from, 'Error!,salah program memek', id)
                                })
                        } 
                    break
case 'randomquran':
...
case 'lirik':
	...
	await aruga.reply(from, mess.wait, id)
        ...
        break
case 'film':
        if (args.length == 0) return aruga.reply(from, `Untuk mencari film\nUsage : ${prefix}film cars`, id)
        rugaapi.film(body.slice(6))
        .then(async(res) => {
            const film = `*Judul Film:* ${body.slice(6)}\n\n*aktor :* ${res.result.aktor}\n\n*bahasa :* ${res.result.bahasa}\n\n*dirilis :* ${res.result.dirilis}\n\n*negara* : _${res.result.negara}_\n\n*kategori* : _${res.result.kategori}_\n\n*tipe* : _${res.result.tipe}_\n\n*sinopsis* : _${res.result.sinopsis}_\n\nBy : Zeus`
            aruga.reply(from, film, id)
        })
        .catch(() => {
            aruga.reply(from, 'Errorr...', id)
        })
           ...
           axios.get(`https://api.arugaz.my.id/api/media/ytsearch?query=${pncri}`)
            .then(async (res) => {
				...
				rugaapi.ymp3(`https://youtu.be/${res.data.result[0].id}`)
                                .then(async(res) => {
                                 if (Number(res.filesize.split(' MB')[0] > 10)) return aruga.reply(from, 'Gagal, ukuran Audio melebihi 10 mb!', id)
...
case 'zeus':
            ...
            if (nyoba2 == undefined) {
                var php2 = 'https://i.ibb.co/NnLdkR6/20210210-144604.jpg'
                } else {
                var php2 = nyoba2
                }
...
case 'tutorialmp4':
            ...
            break
case 'hapusiklan':
            await aruga.sendText(from, menuId.textIklan())
            break
case 'hematkuota':
            await aruga.sendText(from, menuId.textKuota())
            break
case 'ytdownload':
            await aruga.sendText(from, menuId.textYt())
            break
case 'themegoogle':
            await aruga.sendText(from, menuId.textCh())
            break
case 'gombalin':
            await aruga.sendText(from, menuId.textGombal())
            break
case 'igdownload':
            await aruga.sendText(from, menuId.textIg())
            break
case 'sadapwa':
            await aruga.sendText(from, menuId.textWa())
            break
case 'lacakhp':
            await aruga.sendText(from, menuId.textHp())
            break
case 'premium':
case 'daftar':
            await aruga.sendText(from, menuId.textPrem(pushname), id)
            break
          case 'tod':
    case 'game':
...
case 'us':
if (!isPrem) return aruga.reply(from, 'Command Premium!\ngagal! Maaf anda belum terdaftar sebagai user premium\nsilahkan chat owner bot untuk mendaftar.', id)
       if (args.length == 0) return aruga.reply(from, 'Menu buat bicara sama ZeusXzBot gan,\nContoh: #us halo', id)
...
case 'owner':
            ...
            .then(() => aruga.sendText(from, 'silahkan yang mau chat nomer owner ZeusXz-Bot'))
            break
...
case 'randomsticker':                 
 aruga.reply(from, mess.wait, id);
                axios.get('https://akaneko-api.herokuapp.com/api/mobileWallpapers').then(res => {
                    aruga.sendStickerfromUrl(from, res.data.url,'Desktop Wallpaper.jpeg', id);
                });
                break  
...
case 'nekosticker':
               try {
                    aruga.reply(from, mess.wait, id)
                    axios.get('https://akaneko-api.herokuapp.com/api/neko').then(res => {
                        aruga.sendStickerfromUrl(from, res.data.url, 'neko.jpeg');
                    });
                } catch (err) {
                    console.log(err);
                    throw(err);
...
if (!isPrem) return aruga.reply(from, 'Command Premium!\ngagal! Maaf anda belum terdaftar sebagai user premium\nsilahkan chat owner bot untuk mendaftar.', id)
                ...
                case 'jadian':
                    ...
                    const sapa = `CIEEEEE @${aku.replace(/[@c.us]/g, '')} (💘) @${kamu.replace(/[@c.us]/g, '')} HABIS JADIAN WOY NII\nBAGI PJ NYA DONG JAN PELIT PELIT NTR GA LANGGNEG LOH:V\n\nBy : ZeusXz`
                    await aruga.sendTextWithMentions(from, sapa)
                    break     
...
case 'nulis2':
    if (args.length == 0) return aruga.reply(from, `Membuat Nulis Text \nPemakaian: ${prefix}nulis2 [teks]\n\ncontoh: ${prefix}nulis2 ZeusXz`, id)
            ...
            break
case 'nulis7':
    if (args.length == 0) return aruga.reply(from, `Membuat Nulis Text \nPemakaian: ${prefix}nulis7 [teks]\n\ncontoh: ${prefix}nulis7 ZeusXz`, id)
                    await aruga.reply(from, `wait Sedang diproses....`, id)
                console.log('sedang menulis...')
...
if (!isPrem) return aruga.reply(from, 'Command Premium!\ngagal! Maaf anda belum terdaftar sebagai user premium\nsilahkan chat owner bot untuk mendaftar.', id)
        ...
        try {
            ...
            break
      case 'logoml':
            if (args.length === 0) return aruga.reply(from, `Kirim perintah *#logoml |Hero| Text*,\n\n contoh : *#logoml |layla| ZEUSXz*`, id)
            argz = body.trim().split('|')
            if (argz.length >= 2) {
                aruga.reply(from, `Wait sedang diproses...\n\nNote : jika bot mengirim file maka hero yang anda masuukan tidak tersedia,silahkan coba nama hero lain.`, id)
...
case 'zeusff':
            ...
            if (argz.length >= 2) {
                ...
                await aruga.reply(from, `Wrong Format!\n[❗] Kirim perintah *#zeusff |ZeusXz|awas sakit mata*`, id)
            }
 break
case 'quotemaker2':
...
break
case 'animequotes':
const qtosam = await axios.get(`http://api-melodicxt-2.herokuapp.com/api/get/anime-quotes?apiKey=administrator`).then(res => {
    const ftoam = `nama karakter anime : *${res.data.result.anime}*\n\nQuotes : *${res.data.result.quote}*\n\nBy : ZeusXz`;
    aruga.sendText(from, ftoam, id)
})
break
case 'renungan':
await aruga.reply(from, mess.wait, id)
const renungan = await axios.get(`http://api-melodicxt-2.herokuapp.com/api/alkitab/renunganpagi?apiKey=administrator`).then(res => {
    const rg = `*judul/ayat* : ${res.data.result.surah}\n\n*Renungan :* ${res.data.result.content}\n\nBy : ZeusXz`;
...
case 'cersex2':
await aruga.reply(from, mess.wait, id)
const cersex2 = await axios.get(`https://api-melodicxt-2.herokuapp.com/api/cerita-sex?query=sedarah&apiKey=administrator`).then(res => {
    const cersex2f = `*judul* : ~${res.data.categories_info}\n*date :* ${res.data.date}~\n\n*Cerita :* ${res.data.post_message}\n\nBy : ZeusXz`;
    aruga.sendText(from, cersex2f, id)
})
break
case 'stalktiktok':
            case 'stalktik':
...
case 'randomcat':
aruga.reply(from, mess.wait, id)
 const cat = await axios.get(`https://api.vhtear.com/randomcat?apikey=${vhtearkey}`)
            const ct = cat.data.result
            aruga.sendFileFromUrl(from, ct.url, 'cat.jpeg', 'Nih....', id)
                       break
case 'lolistiker':
case 'lolisticker':
aruga.reply(from, mess.wait, id)
   const lolis = await axios.get(`https://api.vhtear.com/randomloli&apikey=${vhtearkey}`)
            const lolys = lolis.data.result
            aruga.sendStickerfromUrl(from, lolys.result, 'lolis.jpeg', id)
                       break
                           case 'asupan':
...
case 'ytmp4':
    if (args.length == 0) return aruga.reply(from, `Untuk mendownload video dari youtube\nketik: ${prefix}ytmp4 [link_yt]`, id)
	    ...
	    rugaapi.ytmp4(`https://youtu.be/${linkmp4}`)
            .then(async(res) => {
				await aruga.sendFileFromUrl(from, res.image, '', `「 *YOUTUBE MP4* 」\n\nJudul: ${res.title}\nSize: ${res.size_video}\nDurasi: ${res.duration}\nRating: ${res.rating}\nViews: ${res.viewcount}\n\n*_Waitt, lemme send this fuckin' video_*\nSetelah vidio dikirim jangan lupa berterimakasih\n\n*Ga makasih = Block!!*\n\nBY : ZeusXz`, id)
                                if (Number(res.size_video.split(' MB')[0] > 10)) return aruga.reply(from, 'Maaf, Ukuran file terlalu besar!', id)
...
case 'alkitab':
            ...
            aruga.reply(from, 'Wait.....', id)
            try {
                const dataplai = await axios.get(`http://api-melodicxt-2.herokuapp.com/api/alkitab/search?query=${alkitabx}&apiKey=administrator`)
                const dataplay = dataplai.data
 ...
 case 'hitunghuruf':
        ...
        case 'kick':
            ...
            for (let i = 0; i < mentionedJidList.length; i++) {
                if (groupAdmins.includes(mentionedJidList[i])) return await aruga.sendText(from, 'woy lapet-lapet admin mana bisa dikick pinter..!!')
...
case 'harta':
             ...
             await aruga.sendFileFromUrl(from, `https://api.zeks.xyz/api/hartatahta?text=${jreng}&apikey=apivinz`,`${jreng}.jpg`,`Harta Tahta ${jreng}`, id)        
             break
        case 'family100':
            try {
...
case 'prem':
            if (!isOwnerBot) return aruga.reply(from, 'Perintah ini hanya untuk Owner bot!', id)
            if (args.length == 0) return aruga.reply(from, `Untuk add premium seseorang agar bisa menggunakan menu premium\n\nCaranya ketik: \n${prefix}prem add 628xx --untuk mengaktifkan\n${prefix}ban del 628xx --untuk nonaktifkan\n\ncara cepat ban banyak digrup ketik:\n${prefix}ban @tag @tag @tag`, id)
            if (args[0] == 'add') {
                prem.push(args[1]+'@c.us')
                fs.writeFileSync('./lib/database/prem.json', JSON.stringify(prem))
                aruga.reply(from, 'Success add premium number!', id)
            } else
            if (args[0] == 'del') {
                let xnxx = banned.indexOf(args[1]+'@c.us')
               prem.splice(xnxx,1)
                fs.writeFileSync('./lib/database/prem.json', JSON.stringify(prem))
                aruga.reply(from, 'Success unpremium number!', id)
            } else {
             for (let i = 0; i < mentionedJidList.length; i++) {
                prem.push(mentionedJidList[i])
                fs.writeFileSync('./lib/database/prem.json', JSON.stringify(prem))
                aruga.reply(from, 'Success premium number!', id)
                }
            }
            break
case 'listpremium':
case 'listprem':
                let premium = `This is list of premium number\nTotal : ${prem.length}\n`
                for (let i of prem) {
                    premium += `➸ ${i.replace(/@c.us/g,'')}\n`
                }
                await aruga.reply(from, premium, id)
                break
             case 'google':
...
if (!isPrem) return aruga.reply(from, 'Command Premium!\ngagal! Maaf anda belum terdaftar sebagai user premium\nsilahkan chat owner bot untuk mendaftar.', id)
 ...
 if (args.length == 0) return aruga.reply(from, `Contoh #kapan saya nikah`, id)
                 ...
                 case 'rate':
                     ...
                     break
case 'howgay':
                     const gay = args.join('@c.us')
                     const awrg = rate[Math.floor(Math.random() * (rate.length))]
                     if (!gay) aruga.reply(from, `⚠️ Format salah! Ketik *${prefix}menu* untuk penggunaan.`, id)
                     await aruga.sendText(from, `Gay Si : *${gay}* \n\nPersen : ${awrg}\n\nBy : ZeusXz`)
                     break
                 case 'apakah':
...
case 'silk':
            ...
            break
case 'pinterest':
if (args.length === 0) return aruga.reply(from, 'Kirim perintah *#images [kata] untuk mencari gambar*', id)
if (args.length === 1) return aruga.reply(from, 'Kirim perintah *#images [kata] untuk mencari gambar*', id)
if (args.length === 2) return aruga.reply(from, 'Kirim perintah *#images [kata] untuk mencari gambar*', id)
if (args.length === 3) return aruga.reply(from, 'Kirim perintah *#images [kata] untuk mencari gambar*', id)
if (args.length === 4) return aruga.reply(from, 'Kirim perintah *#images [kata] untuk mencari gambar*', id)
if (args.length === 5) return aruga.reply(from, 'Kirim perintah *#images [kata] untuk mencari gambar*', id)
break
...
case 'images':
          aruga.reply(from, mess.wait, id)
           const ptrsq = body.slice(8)
            ...
            await aruga.sendImage(from, base64, 'ptrs.jpg', `_Hasil Searching :_ *${ptrsq}*\n\n*Images By : ZeusXz-Bot*`)
               break
...
case 'memeindo2':
aruga.reply(from, mess.wait, id)
            const memeindox2 = await rugaapi.memeindo2()
            await aruga.sendFileFromUrl(from, memeindox2, 'memeindo2.jpg', 'Nih.....', id)
            .catch(() => {
                aruga.reply(from, 'Hayolohhh, ada yang error!!', id)
            })
            break
...
case 'textgplay':
 ...
 case 'twitterstalk':
        case 'twtstalk':
            if (args.length === 0)  return aruga.reply(from, 'Kirim perintah *#twtstalk @username*\nContoh *#twtstalk @miakhalifah*', id)
            argz = body.trim().split(' ')
            console.log(...argz[1])
            var slicedArgs = Array.prototype.slice.call(arg, 1);
            console.log(slicedArgs)
...
➸ *Wilayah:* +${region_name}\n\nBy : Zeus`
        ...
        case 'bc':
            if (!isOwnerB) return aruga.reply(from, `Perintah ini hanya untuk Owner  ZeusXz`, id)
                bctxt = body.slice(4)
                txtbc = `〘 *ZeusXz BC* 〙\n\n${bctxt}`
                ...
                if(cekuser && isIn !== false){
                    ...
                    if(cekMember.length === 0){
                        ...
                        if(isKasar){
                            ...
                            await aruga.reply(from, "Jangan toxic bodoh!\ntobat woy lapet lapet..\nDenda +10.000", id)
                        } else {
